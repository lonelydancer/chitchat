...
from model.ernie_v1 import ErnieConfig
from model.optimization import optimization
from utils.init import init_model
from utils.args import print_arguments
from finetune.chitchat import ErnieLMFinetune
...
def main(args):
    ...
    ernie_lm = ErnieLMFinetune(args, ernie_config, reader.tokenizer)
    print ('hello')
    ...
    if args.do_train:
        ...
        with fluid.program_guard(train_program, startup_prog):
            with fluid.unique_name.guard():
                train_pyreader, graph_vars = ernie_lm.create_model()
                print ('create end')
    ...
    if args.do_val or args.do_test or args.do_pred:
        ...
        with fluid.program_guard(test_prog, startup_prog):
            with fluid.unique_name.guard():
                test_pyreader, test_graph_vars = ernie_lm.create_model()
    ...
    if args.do_val or args.do_test or args.do_pred:
        ...
        eval_func = partial(ernie_lm.evaluate, resource=test_resource, graph_vars=test_graph_vars,
        ...
        while True:
            try:
                ...
                if steps % skip_steps == 0:
                    outputs = ernie_lm.evaluate(train_resource, "train", graph_vars)
                    if args.verbose:
                    ...
                    x = [graph_vars['sent_ids'].name]#,'encoder']
                    labels = train_exe.run(fetch_list=x)
                    print (labels)
